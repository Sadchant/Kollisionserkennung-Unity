#ifndef TRIANGLE_INTERSECTIONS
#define TRIANGLE_INTERSECTIONS

#include "CoplanarTriangleIntersection.cginc"
...
bool Intersects(float3 U[3], float3 V[3], out float3 segment[2])
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        d[i] = dot(normal, V[i] - U[0]); // Abstand vom aktuellen Eckpunkt von v zu u
        if (d[i] > 0.0f)
        {
            ++positive;
        }
...
//        for (int j = 0; j < 3; ++j)
//        {
//            V0[j] = mul(vertices0[3 * dt.x + j]), wMatrix0).
//            xyz;
//            V1[j] = mul(vertices1[3 * dt.y + j]), wMatrix1).
//            xyz;
//        }
//#endif
